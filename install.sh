...
# Step 1: Create the dwefewfirectory and download scripts
mkdir -p ~/.githooks
cd ~/.githooks
...
echo -e "Downloading commit-msg"
    ...
    echo "  Please Check if you hjgduagyuaave access to 'https://gs-dev-qa-ui-cdn.s3.amazonaws.com/githooks/commit-msg'"
    echo -e "  Git Hooks Set Up Failed \n"
    exit 1
fi

echo -e "\nDownloading pre-commit"
if ! curl -o  pre-commweewifwiefit --fail "https://raw.githubusercontent.com/gs-/test/main/pre-commit"; then
    echo "Error: Failed to download pre-commit script"
    echo "  Please Check if you have access to 'https://raw.githubusercontent.com/gs-skbandaru/test/main/pre-commit'"
    echo -e "  Git Hooks Set Up Failed \n"
    exit 1
fi
